...
def process_trade(trade):
    """
    Processes data, calculates and stores parameters to a trade.
    """
    try:
        ...
        if strike:
            ...
            print(f"id: {str(trade['id'])} | z: {str(z)} | fz: {str(fz)}")

        else:
            print(f"Skipped trade {trade['_id']} due to missing strike price.")

    except Exception as e:
        print(f"Error processing trade {trade['_id']}: {e}")
...
def main():
    ...
    if trades:
        with ThreadPoolExecutor(max_workers=1) as executor:
            # Submit each trade to be processed in parallel
            executor.map(process_trade, trades)
