####
#### Prospects short straddle opportunities with BTC options on Deribit.
####
...
opp_straddle_collection = db['btc_day_options_opportunities_straddle']
...
have_documents = opp_straddle_collection.find_one(sort=[('datetime', -1)])
if have_documents is None:
    opp_straddle_collection.create_index([('datetime', DESCENDING)])
    opp_straddle_collection.create_index([('datetime', ASCENDING)])
...
for instrument in active_instruments:
    ...
    ## The PUT instrument.
    p_instrument = instrument_collection.find_one({
        'is_active': True,
        'option_type': "put",
        'expiration_unix_timestamp': c_instrument['expiration_unix_timestamp'],
        'strike': c_instrument['strike'],
        })   

    # Shows the instrument pair.
    if p_instrument is not None:
        ...
        ###
        ### Calculating the CALL fair price
        ###

        # Gets t (the time left to expiration).
        t = c_instrument['expiration_unix_timestamp'] - td_unix

        # Gets life-time ratio (in milliseconds):
        lifetime_ratio = (
                    (td_unix - c_instrument['creation_unix_timestamp']) / 
                    (c_instrument['expiration_unix_timestamp'] - 
                     c_instrument['creation_unix_timestamp']))

        # Gets x:
        x = base_index_price / c_instrument['strike']
        ...
        z = mp.mpf(z_model['k1'] + 
                   ...
                   (z_model['k4'] * e2) + 
                   (z_model['k5'] * x) + 
                   (z_model['k6'] * c_instrument['strike']))
        ...
        if y:
            Ycalc = y * c_instrument['strike']
            Y = math.floor(Ycalc * 1e4 ) / 1e4 # Rounds Ycalc to 4 decimals.
