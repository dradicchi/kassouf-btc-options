###
### Builds the BTC inverse options offer database.
###
...
collection = db['btc_inverse_options_offer']
...
if hist_response.status_code == 200:
    ...
    for item in hist_instrument_list:
        ...
        is_doc_stored = collection.find_one(query)

        if is_doc_stored is None:

            creation_dt = datetime.fromtimestamp((int(item['creation_timestamp']) / 1000.0))
            expiration_dt = datetime.fromtimestamp((int(item['expiration_timestamp']) / 1000.0))
...
if prod_response.status_code == 200:
    ...
    for item in prod_instrument_list:
