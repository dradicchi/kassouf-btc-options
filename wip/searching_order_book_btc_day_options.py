...
for instrument in active_instruments:
    ...
    z_model_list = z_model_collection.find({'instrument_cycle': "day"})
        ...
        if bids:
            for bid in bids:
                ...
                if YdivY:
                    ...
                    # Iterates all z models:
                    #adj_model_list = adj_model_collection.find()

                    #for adj_model in adj_model_list:

                    if ((float(3) <= YdivY <= float("+inf")) and
                        (float(0.45) <= life_time_portion <= float(0.80)) and
                        #(float(0.3) <= logXdivXavg <= float(0.6))): #and
                        #(float(100) <= current_order_book['result']['mark_iv'] <= float("+inf"))): #and
                        (float(0.96) <= x <= float("+inf"))): #and
